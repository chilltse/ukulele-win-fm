...
def parse_train_metrics(text: str) -> Dict[str, str]:
    """
    Parse the final metric row printed by wandb_*_train.py.
    ...
    Example:
        0 akt qid_tree 0.7234 0.6812 0.7123 0.6744 0.7301 0.6902 18
    """
...
def str2bool_or_raw(value: Optional[str]) -> Optional[str]:
    """
    Keep bool-like values as lowercase strings because downstream training scripts
    may parse them differently. This function mainly normalizes common spelling.
    """
    if value is None:
        return None
    v = str(value).strip()
...
def resolve_train_script(examples_dir: Path, model_name: str, train_script_arg: str) -> Path:
    """
    Resolve the training script.

    Default:
        model_name=dkt -> examples/wandb_dkt_train.py
        model_name=akt -> examples/wandb_akt_train.py

    You can override with --train_script.
    """
    if train_script_arg:
        p = Path(train_script_arg)
        if not p.is_absolute():
            p = examples_dir / p
        return p

    return examples_dir / f"wandb_{model_name}_train.py"


TREE_ARG_NAMES = [
    "tree_embed_mode",
    "tree_aux_loss_weight",
    "tree_aux_loss_mode",
    "tree_aux_max_depth",
    "tree_aux_depths",
    "tree_pred_fusion_mode",
    "tree_pred_fusion_weight",
    "tree_pred_max_depth",
    "tree_pred_depths",
    "tree_aux_ignore_first",
    "tree_max_ancestor_depth",
]


def collect_tree_args(args: argparse.Namespace, emb_type: str) -> List[str]:
    """
    Collect qid_tree-specific CLI args for wandb_*_train.py.

    Important:
        We only pass these args when emb_type == qid_tree.
        This prevents the qid baseline from failing when its training parser
        does not recognize qid_tree-specific options.
    """
    if emb_type != "qid_tree":
        return []

    out: List[str] = []
    for name in TREE_ARG_NAMES:
        value = getattr(args, name, None)
        if value is None:
            continue
        if name in {"tree_aux_ignore_first"}:
            value = str2bool_or_raw(value)
...
def build_train_cmd(
    ...
    # Some generic training scripts require --model_name, but many model-specific
    # scripts such as wandb_akt_train.py do not. Keep it opt-in to avoid breaking
    # model-specific scripts with "unrecognized argument: --model_name".
    if args.pass_model_name_arg:
        cmd.extend(["--model_name", args.model_name])

    cmd.extend(collect_tree_args(args, emb_type))
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Grid-run assist2017 top-k KC coverage split/preprocess/train pipeline for DKT/AKT."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="dkt",
        help="Model name. Example: dkt or akt. Used to choose wandb_<model_name>_train.py by default.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Optional training script path. Default: examples/wandb_<model_name>_train.py.",
    )

    parser.add_argument(
        "--pass_model_name_arg",
        action="store_true",
        help="Also pass --model_name to the training script. Use this only if your script accepts/requires it.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="qid_tree",
        help="Embedding type for the main tree dataset job. Example: qid_tree or qid.",
    )

    # ---------------- qid_tree arguments forwarded to wandb_*_train.py ----------------
    parser.add_argument("--tree_embed_mode", type=str, default=None)
    parser.add_argument("--tree_aux_loss_weight", type=float, default=0.4)
    parser.add_argument("--tree_aux_loss_mode", type=str, default=None)
    parser.add_argument("--tree_aux_max_depth", type=int, default=None)
    parser.add_argument("--tree_aux_depths", type=str, default=None)
    parser.add_argument("--tree_pred_fusion_mode", type=str, default=None)
    parser.add_argument("--tree_pred_fusion_weight", type=float, default=None)
    parser.add_argument("--tree_pred_max_depth", type=int, default=None)
    parser.add_argument("--tree_pred_depths", type=str, default=None)
    parser.add_argument("--tree_aux_ignore_first", type=str, default=None)
    parser.add_argument("--tree_max_ancestor_depth", type=int, default=None)

    parser.add_argument(
        "--extra_train_args",
        nargs=argparse.REMAINDER,
        default=None,
        help="Extra arguments appended to every training command. Put this at the end of the command.",
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Also train baseline assist2017 + qid using the same model_name/train_script.",
    )
    ...
    args.model_name = args.model_name.strip().lower()
    ...
    train_script = resolve_train_script(examples_dir, args.model_name, args.train_script)
    ...
    print(f"pass_model_name_arg: {args.pass_model_name_arg}")
    ...
    try:
        ...
        for k in args.topk:
            ...
            row_base: Dict[str, str] = {
                "topk": str(k),
                "model_name_requested": args.model_name,
            ...
            try:
                ...
                for train_job in train_jobs:
                    ...
                    for fold in folds:
                        print(
                            f"[{tag}] Step 6: train model={args.model_name}, "
                            f"dataset={dataset_name}, emb_type={emb_type}, fold={fold}"
                        )

                        train_log_file = (
                            run_dir
                            / f"train_{safe_filename(args.model_name)}_{safe_filename(dataset_name)}_{safe_filename(emb_type)}_fold{fold}.log"
                        )
...
# python .\assist2017_topk_grid_pipeline_akt.py ^
#   --model_name akt ^
#   --emb_type qid_tree ^
#   --tree_aux_loss_weight 0.3 ^
#   --tree_aux_loss_mode direct ^
#   --tree_pred_fusion_mode none ^
#   --also_train_assist2017_qid ^
#   --folds 0
    main()
