...
class DKT(Module):
    def __init__(
        ...
        tree_pred_fusion_mode="none", # none / fixed / frequency：控制预测阶段是否融合 ancestor prior。
        tree_pred_fusion_max_weight=0.4, # ancestor prior 最大话语权，建议 0.2~0.5。
        tree_pred_fusion_fixed_weight=0.2, # fixed 模式下的固定融合权重。
        tree_pred_fusion_count_tau=50.0, # frequency 模式下，KC 样本数衰减温度。默认50
        tree_pred_fusion_depth_decay=None, # ancestor prediction 融合时的深度衰减；None 表示复用 tree_aux_decay。
        tree_pred_fusion_counts_path="", # 可选：KC 训练样本数 json/list 文件路径。
    ):
        super().__init__()
...
#################
    ...
    def get_qid_tree_target_prediction(self, y, q, r=None, use_fusion=True, return_details=False):
        """
        ...
        ancestor_prior, has_ancestor, _ = self._qid_tree_ancestor_prior(pred_all, target_q)
    ...
    def apply_qid_tree_prediction_fusion(self, y, q, r=None, return_details=False):
        """
    ...
    def forward(self, q, r, q_dense, apply_tree_pred_fusion=None):
        ...
        if self.emb_type == "qid_fmkc":
            ...
            if apply_tree_pred_fusion is None:
                apply_tree_pred_fusion = self.tree_pred_fusion_mode != "none"
            if apply_tree_pred_fusion:
                y = self.apply_qid_tree_prediction_fusion(y, q, r=r, return_details=False)
