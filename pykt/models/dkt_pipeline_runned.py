...
class DKT(Module):
    def __init__(
        ...
        dpath="",
        kc_tree_path="",
        tree_aux_loss_weight=None, # qid_tree: must be explicitly set by training entry.
        tree_aux_decay=None, # qid_tree: must be explicitly set by training entry.
        tree_aux_max_depth=None, # qid_tree: must be explicitly set by training entry.
        ...
        tree_pred_fusion_max_weight=None, # qid_tree: must be explicitly set by training entry.
        ...
        tree_pred_fusion_depth_decay=None, # ancestor prediction 融合时的深度衰减；None 表示复用 tree_aux_decay。
        ...
        if emb_type == "qid_fmkc":
            ...
            self.tree_pred_fusion_depth_decay = (
            ...
            valid_fusion_modes = {"none", "fixed", "frequency"}
            valid_aux_gradient_modes = {"shared", "detached"}
            valid_fusion_sources = {"main", "aux"}
            ...
            if self.tree_aux_gradient_mode not in valid_aux_gradient_modes:
                raise ValueError(
                    f"tree_aux_gradient_mode must be one of {sorted(valid_aux_gradient_modes)}, "
                    f"got {self.tree_aux_gradient_mode!r}"
                )
            if self.tree_pred_fusion_source not in valid_fusion_sources:
                raise ValueError(
                    f"tree_pred_fusion_source must be one of {sorted(valid_fusion_sources)}, "
                    f"got {self.tree_pred_fusion_source!r}"
                )
            ...
            if self.tree_pred_fusion_depth_decay < 0:
                raise ValueError("tree_pred_fusion_depth_decay must be >= 0")
            ...
            print(f"tree_pred_fusion_depth_decay = {self.tree_pred_fusion_depth_decay}")
            print(f"tree_aux_gradient_mode = {self.tree_aux_gradient_mode}")
            print(f"tree_pred_fusion_source = {self.tree_pred_fusion_source}")
            print(f"tree_pred_fusion_apply_train = {self.tree_pred_fusion_apply_train}")
            if self.tree_pred_fusion_mode == "frequency" and float(self.kc_train_counts.sum().item()) <= 0:
                print("warning: frequency fusion is enabled but kc_train_counts are all zero.")
                print("         Call model.set_qid_tree_train_counts(...) or pass tree_pred_fusion_counts_path.")
            print("prediction fusion = ENABLED only when tree_pred_fusion_mode != 'none'")
            print("====================================================\n")
        ...
        # Separate auxiliary head for fully decoupled qid_tree parent supervision.
        # In detached mode, aux predictions are computed from h.detach(), so the
        # ancestor loss trains this head without updating the shared LSTM or the
        # main leaf output head.
        if emb_type == "qid_tree":
            self.tree_aux_out_layer = Linear(self.hidden_size, self.num_c)
            self._last_qid_tree_aux_y = None
...
#################
    ...
    def _qid_tree_ancestor_prior(self, pred_all, target_q):
        """
        Compute the exponentially depth-weighted ancestor prediction prior.
        ...
        Returns:
            ancestor_prior: [B, L-1]
            has_ancestor: [B, L-1] bool
            depth_weight_sum: [B, L-1]
        """
        ...
        raw_weight = torch.exp(-self.tree_pred_fusion_depth_decay * anc_depth)
        raw_weight = raw_weight * anc_valid.float()

        weight_sum = raw_weight.sum(dim=-1)
        has_ancestor = weight_sum > 0

        ancestor_prior = (anc_pred * raw_weight).sum(dim=-1) / weight_sum.clamp(min=1e-12)
        ancestor_prior = torch.where(has_ancestor, ancestor_prior, torch.zeros_like(ancestor_prior))

        return ancestor_prior, has_ancestor, weight_sum
    ...
    def get_qid_tree_target_prediction(self, y, q, r=None, use_fusion=True, return_details=False, fusion_source_y=None):
        ...
        source_pred_all = pred_all
        if self.tree_pred_fusion_source == "aux":
            if fusion_source_y is None:
                fusion_source_y = self._last_qid_tree_aux_y
            if fusion_source_y is None:
                raise RuntimeError(
                    "tree_pred_fusion_source='aux' requires aux predictions. "
                    "Call forward() first so _last_qid_tree_aux_y is set."
                )
        ...
        ancestor_prior, has_ancestor, _ = self._qid_tree_ancestor_prior(source_pred_all, target_q)
        gate = self._qid_tree_fusion_gate(target_q, valid, has_ancestor)
        fused_pred = (1.0 - gate) * leaf_pred + gate * ancestor_prior
    ...
    def get_qid_tree_loss(self, y, q, r, return_details=False):
        ...
        # Parent auxiliary loss may use either the shared main predictions
        # or a fully decoupled auxiliary head trained on h.detach().
        aux_pred_all = pred_all
        if self.tree_aux_gradient_mode == "detached":
            aux_y = self._last_qid_tree_aux_y
            if aux_y is None:
                raise RuntimeError(
                    "tree_aux_gradient_mode='detached' requires aux predictions. "
                    "Call forward() before get_qid_tree_loss()."
                )
            if aux_y.shape != y.shape:
                raise ValueError(
                    f"Stored aux_y shape {tuple(aux_y.shape)} does not match y shape {tuple(y.shape)}"
                )
            aux_pred_all = aux_y[:, :-1, :].clamp(min=eps, max=1.0 - eps)
    ...
    def forward(self, q, r, q_dense, apply_tree_pred_fusion=None):
        ...
        if self.emb_type == "qid_fmkc":
            ...
            # Separate aux prediction head for fully decoupled parent supervision.
            # Detaching h prevents ancestor aux loss from updating the shared LSTM.
            aux_y = self.tree_aux_out_layer(h.detach())
            aux_y = torch.sigmoid(aux_y)
            self._last_qid_tree_aux_y = aux_y
            ...
            if apply_tree_pred_fusion:
                fusion_source_y = aux_y if self.tree_pred_fusion_source == "aux" else None
