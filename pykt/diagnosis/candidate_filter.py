...
""".strip()
    ...
    if n_over > 0:
        ...
        available_cols = [c for c in cols if c in over_coarse_df.columns]
        print(over_coarse_df[available_cols].head(20).to_string(index=False))
    ...
    if n_combo > 0:
        ...
        available_cols = [c for c in cols if c in combination_sensitive_df.columns]
        print(combination_sensitive_df[available_cols].head(20).to_string(index=False))

    return {
        "output_dir": str(output_path),
        "all_flagged": str(all_flagged_path),
        "over_coarse": str(over_coarse_path),
        "combination_sensitive": str(combination_sensitive_path),
