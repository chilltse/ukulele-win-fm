...
def build_tree_concepts_mapping(dkeyid2idx, kc_tree_path):
    """
    Build a tree-aware concept mapping:
        observed leaf concepts + internal parents + optional roots.
    Keep original observed concept ids unchanged, and append new tree-only nodes.
    """
    concepts_map = dkeyid2idx.get("concepts", {})
    if not concepts_map:
        return None
    if not kc_tree_path or not os.path.exists(kc_tree_path):
        return None
    ...
    for item in tree_data.get("kc_index", []):
        ...
        if kc_id is not None:
            tree_kc_ids.add(int(kc_id))

    merged = dict(concepts_map)
    next_id = len(merged)
    for kc_id in sorted(tree_kc_ids):
        raw = str(kc_id)
...
def write_config(dataset_name, dkeyid2idx, effective_keys, configf, dpath, k=5, min_seq_len=3, maxlen=200, flag=False, other_config={}, keyid2idx_tree=None):
    ...
    if keyid2idx_tree is not None and "concepts" in keyid2idx_tree:
        dconfig["num_c_tree"] = len(keyid2idx_tree["concepts"])
...
def main(dname, fname, dataset_name, configf, min_seq_len=3, maxlen=200, kfold=5):
    ...
    keyid2idx_tree = None
    if "kc_tree_path" in other_config:
        keyid2idx_tree = build_tree_concepts_mapping(
            dkeyid2idx=dkeyid2idx,
            kc_tree_path=other_config["kc_tree_path"],
        )
